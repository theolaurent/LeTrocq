...
The minimal classes are computed by a dedicated **solver** over the type's dependency structure,
using the tables in `Lattice.lean` (`depPi`, `depArrow`, `depType`). Grades are written `@(m,n)`.
