...
The ARROW construction: building `Param … (A→B) (A'→B')` from witnesses for the parts.
  ...
  • `arrowCov`/`arrowContra` + `paramArrow` — the GRADED family: arrow at every output class incl. (4,4)
    (the (4,4) coherence free by `Map4Has.subsingleton`), with parts at the `depArrow`-minimal classes.
...
def arrowCov {A B A' B' : Type u} :
    (m : MapClass) →
    (pa : Param (mapDepArrow m).1.1 (mapDepArrow m).1.2 A A') →
    (pb : Param (mapDepArrow m).2.1 (mapDepArrow m).2.2 B B') →
...
def arrowContra {A B A' B' : Type u} :
    (n : MapClass) →
    (pa : Param (mapDepArrow n).1.2 (mapDepArrow n).1.1 A A') →
    (pb : Param (mapDepArrow n).2.2 (mapDepArrow n).2.1 B B') →
...
def paramArrow {A B A' B' : Type u} (m n : MapClass)
    (pa : Param (depArrow (m, n)).1.1 (depArrow (m, n)).1.2 A A')
    (pb : Param (depArrow (m, n)).2.1 (depArrow (m, n)).2.2 B B') :
    Param m n (A → B) (A' → B') where
  R := RArrow pa.R pb.R
  cov := arrowCov m
    ((pa.weaken (by cases m <;> cases n <;> rfl) (by cases m <;> cases n <;> rfl)) :
      Param (mapDepArrow m).1.1 (mapDepArrow m).1.2 A A')
    ((pb.weaken (by cases m <;> cases n <;> rfl) (by cases m <;> cases n <;> rfl)) :
      Param (mapDepArrow m).2.1 (mapDepArrow m).2.2 B B')
  contra := arrowContra n
    ((pa.weaken (by cases m <;> cases n <;> rfl) (by cases m <;> cases n <;> rfl)) :
      Param (mapDepArrow n).1.2 (mapDepArrow n).1.1 A A')
    ((pb.weaken (by cases m <;> cases n <;> rfl) (by cases m <;> cases n <;> rfl)) :
