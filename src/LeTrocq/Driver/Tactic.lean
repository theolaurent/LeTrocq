...
elab_rules : term
  | `(transfer% $t $[to $tgt?]?) => do
      ...
      let wit ← Driver.Transfer.transfer (← instantiateMVars tE) (map4, map4) target?
...
elab_rules : tactic
  | `(tactic| trocq $[to $tgt?]?) => do
      ...
      let wit ← Driver.Transfer.transfer goalTy (map0, map1) target?
...
elab "relate% " t:term : term => do
  let e ← Lean.Elab.Term.elabTerm t none
  synthesizeSyntheticMVarsNoPostponing
  instantiateMVars (← Driver.Transfer.relate (← instantiateMVars e))
