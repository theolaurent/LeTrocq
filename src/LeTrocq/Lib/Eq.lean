...
of the correspondence to transport the equality. Because `a = b` / `a' = b'` are PROPOSITIONS, the
parametricity relation `EqR` is trivial (`PLift True`) and the witness carries no data above class 1 — so,
like every other relator, `Eq` is GRADED: to transport the equality in one direction it needs only the
underlying type's COMPLETENESS (`rInMap`, class 2b) in that direction, never a full equivalence. The maximum
demand is `(2b,2b)` (both directions); a one-directional transport — e.g. the `trocq` goal seed `(0,1)` —
needs only `(0,2b)`. (Contrast `List`/`Prod`, whose element needs only the bare relation: `Eq` transports an
equation, so it needs the maps' completeness, but still stops short of a bijection.)
...
/-- the parametricity relation of `Eq`: since `a = b` / `a' = b'` are propositions (subsingletons),
    relatedness carries no data — the trivial `PLift True`. Named for parity with `ListR`/`SigmaR`; `paramEq`
    uses it as its carried relation, and the transport content lives entirely in the `cov`/`contra` maps. -/
def EqR {A A' : Type} (a b : A) (a' b' : A') : (a = b) → (a' = b') → Type :=
  fun _ _ => PLift True
...
/-- the covariant half `MapHas m (EqR …)` from the underlying type at `mapEqVariance m`: empty below class 1,
    the forward transport `eqFwd` at class ≥ 1 (every higher `Prop` field is free via `propMapHas`). -/
def eqCov {A A' : Type} {a b : A} {a' b' : A'} :
    (m : MapClass) →
    (pa : Param (mapEqVariance m).1 (mapEqVariance m).2 A A') →
    pa.R a a' → pa.R b b' → MapHas m (EqR a b a' b')
  | map0,  _,  _,    _    => {}
...
/-- the contravariant half `MapHas n (flip (EqR …))` from the type's contra at `mapEqVariance n` — the mirror. -/
def eqContra {A A' : Type} {a b : A} {a' b' : A'} :
    (n : MapClass) →
    (pa : Param (mapEqVariance n).2 (mapEqVariance n).1 A A') →
    pa.R a a' → pa.R b b' → MapHas n (fun (h' : a' = b') (h : a = b) => EqR a b a' b' h h')
  | map0,  _,  _,    _    => {}
