...
/- `transfer` exposes the relatedness witness; forward map = native function transport over the `Nat ≃ Unary`
   base, and it COMPUTES: -/
...
/- the witness relates the two function types by the base's respectful relation: -/
...
/- the PAYOFF: a `Prop`-valued dependent goal. `trocq` recognizes `∀ (x : Base), Pred x` via the generic `app`
   rule (registered `Pos`), transfers to the `Nat`-side, and leaves the easier goal. -/
example : ∀ u : Unary, Pos u := by
  trocq                       -- ⊢ ∀ n : Nat, Pos' n
  exact fun n => Nat.zero_le n

/- NO HARDCODING: a predicate registered with `@[trocq]` right here (not in the library) is picked up
   automatically — the driver reads its registries from the `@[trocq]` env extension. -/
...
/- MULTI-ARG relators (`app`-of-`app`): a binary predicate over two base binders transfers — the driver
   handles `head x₁ … xₙ`, applying the relator to every binder's triple. -/
...
/- REVERSE of a HETEROGENEOUS relator: a goal headed by the relator's B-side head (`Pos'`/`Pos2'`, the `Nat`
   side) transfers back — `buildConsts` registers the reverse of `PosR`/`Pos2R` under that head (via
   `symRelator`). -/
...
/- NESTED arguments: a relator argument can be an arbitrary term over the base, not just a bound variable.
   `Pos (Unary.s u)` transfers — the driver rebuilds `Unary.s u` natively as `Nat.succ n` (with relatedness),
   then applies the relator. -/
example : ∀ u : Unary, Pos (Unary.s u) := by
  trocq                       -- ⊢ ∀ n : Nat, Pos' (Nat.succ n)
  exact fun n => Nat.zero_le _

/- LAMBDA arguments: a higher-order predicate whose argument is a λ. `HOpred (fun u => u.s)` transfers, the λ
   rebuilt natively as `fun n : Nat => Nat.succ n` — full transport. -/
...
/- TYPE-VARIABLE arguments: a relator argument mentioning a `∀ A : Type`-bound type variable. The driver
   threads the bound type variable into the term translation and supplies the type's own `Param` witness to
   the relator — full transport under a universe binder. -/
...
/- COMPOUND type arguments: the relator's type parameter is `T := A → A` (an arrow over the bound type var),
   which the solver builds recursively — `IsInhabR` receives that `Param (A → A) (A' → A')` as its type
   witness. -/
...
/- PARAMETERIZED TYPES: `transfer from (List Nat)` lifts the base through `paramList`; forward map =
   `List.map` over the base map, and it COMPUTES. -/
...
/- DEPENDENT Π over a PARAMETERIZED-TYPE binder: `∀ l : List Unary, P l`. The driver builds the domain witness
   (`paramList`) for any domain type, not just a bare base — the bound `l` flows into the body's `app` node
   with that witness as its relatedness. -/
...
/- DEPENDENT type formers: the relator framework handles a type FAMILY argument (`Sigma`'s `β`), built as a
   family of `Param`s. `transfer from (Σ _ : Nat, Nat)` lifts the base through `paramSigma`; the dependent-pair
   forward map COMPUTES. -/
example :
    (transfer from (Σ _ : Nat, Nat)).cov.map ⟨Nat.zero, Nat.succ Nat.zero⟩ = ⟨Unary.z, Unary.s Unary.z⟩ := rfl

/- and a `Prop` goal binding a `Σ`-typed variable transfers — the domain `Σ _ : Unary, Unary` built via
   `paramSigma` (the family machinery feeding `piTerm`). -/
...
/- W-types: a goal binding a `WTree`-typed variable transfers, its domain built via the recursive
   `paramWTree`. -/
...
/- QUOTIENTS: `Quot r` is a former over a type and a relation (a term arg). `paramQuot` builds
   `Quot r ≃ Quot r'` (maps are `Quot.lift`s); the forward map COMPUTES. -/
...
/- CARTESIAN PRODUCT: `paramProd` lifts the base through both parameters; forward map acts componentwise and
   COMPUTES. (`Bool` rides along diagonally in the mixed `Nat × Bool`.) -/
...
/- BOOL takes the WHOLE-DIAGONAL short-circuit: `Bool`'s counterpart is itself, so `assemble` builds the
   generic `paramRefl` (no per-type registration). `transfer from (Bool → Bool)` transports a function over
   that diagonal — the forward map is the function itself. -/
example : (transfer from (Bool → Bool)).cov.map (fun b => !b) true = false := rfl

/- EMPTY / UNIT (in `Type`): also unregistered, taking the same whole-diagonal short-circuit — they transfer
   as leaves and compose with the formers. -/
...
/- `Tw A C B` carries a phantom type parameter `C` between the family's domain `A` and the family `B`, and is
   just `Sigma B`, so its relator delegates to `paramSigma`. It exists only to exercise the driver's
   non-adjacent-family-domain routing, so it lives here (not in `Examples/`). -/
...
/- NON-ADJACENT family domain: `Tw Nat Unary β` puts a phantom `C := Unary` between the family's domain
   ...
   type arg (`Unary`), so `β` gets the `Nat ≃ Unary` witness — and the dependent map COMPUTES. -/
example :
    (transfer from (Tw Nat Unary (fun _ : Nat => Nat))).cov.map ⟨Nat.zero, Nat.succ Nat.zero⟩
      = ⟨Unary.z, Unary.s Unary.z⟩ := rfl

/- EQUALITY: `paramEq` relates `a = b` to `a' = b'` over a related type (its first argument carries the whole
   `Param`, not a bare relation). Both surfaces cross `=`: `transfer`/`trocq` (type/goal) and `relate` (term,
   `[a = b] : PLift (a=b ↔ a'=b')` projected off the witness). -/
...
/- CONNECTIVES IN GOALS: `And`/`Or`/`Not`/`Iff` are graded relators, so the solver crosses them — a goal
   headed by a connective transfers, its `Prop` parts each recursing as a `Param` component (like `Prod`).
   Nesting works. -/
