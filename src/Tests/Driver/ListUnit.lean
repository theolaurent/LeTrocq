...
example : (transfer% (List Unit)).cov.map [(), (), ()] = 3 := rfl
example : (transfer% (List Unit)).contra.map 0 = ([] : List Unit) := rfl
-- CHECK mode with an explicit target.
example : (transfer% (List Unit) to Nat).cov.map [()] = 1 := rfl
-- nested inside a relator argument.
example : (transfer% (List Unit → Bool)).cov.map (fun l => l.length == 0) 0 = true := rfl
