...
/- the section keeps cov's map and the backward map (contra is map2b here). -/
example : (RN.toSection.cov).map = Unary.ofNat := rfl
...
/- a class-4 relation is a subsingleton (`Map4Has.subsingleton`), which makes the (4,4) coherence free. -/
